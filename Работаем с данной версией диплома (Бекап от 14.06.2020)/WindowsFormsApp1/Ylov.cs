...
namespace WindowsFormsApp1
{
    public partial class Hamsa : Form
    {
        ...
        private void Hamsa_Load(object sender, EventArgs e)
        {
            if (readOnl)
            {
                уловыDataGridView.ReadOnly = readOnl;
                обновитьВычесляемыеПараметрыToolStripMenuItem.Enabled = false;
                обновитьЗначенияВыбранногоУловаToolStripMenuItem.Enabled = false;
                bindingNavigatorDeleteItem.Enabled = false;
                button1.Enabled = false;
                уловыBindingNavigatorSaveItem.Enabled = false;
        ...
        private void обновитьЗначенияВыбранногоУловаToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                //тут должен быть код обновления значений !!!!!!!!!!!!!
                 int j = Convert.ToInt32(уловыDataGridView.CurrentCell.RowIndex);
                string catc = Convert.ToString(уловыDataGridView[0, j].Value);
                string sample = Convert.ToString(уловыDataGridView[1, j].Value); //с 17 по 31.
                ArrayList znachs = new ArrayList();
                for (int i = 17; i <= 31; i++)
